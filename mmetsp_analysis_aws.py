...
#description	:python wrapper for de novo MMETSP transcriptome assembly
...
import os, sys, glob, subprocess, getopt, datetime
...
cmd = "awk 'NR==1{print $2/1048576-0.5}' /proc/meminfo"  # keep mem for misc processes
mem_proc = subprocess.check_output(cmd, shell=True)
MEM_TOT = mem_proc[0]
cpu_proc = subprocess.Popen('nproc', stdout=subprocess.PIPE) # CPU number from nproc
...
def buildProjectDir(m_id):
	...
	dirs2 = ['ncgr','trim','trim.diginorm','trim.diginorm.deconseq','trinity',\
	'trinity.pickH','trinity.pickH.cap3.cdhit','trinity.pickH.cap3.cdhit.chimera']
	for dir2 in dirs2:
...
def callDigiNorm(pe,Cval,kval,Nval,xval):
	""" single-pass digital normalization (arXiv: http://bit.ly/1ifqPGs) using \
	recommended parameters. Script normalize-by-median.py is part of the khmer suite \
	(GitHub: http://bit.ly/PLHMB8)."""
...
def fancyPlots(pe,outdir):
	...
	for plot in plotlist:
		if plot in ('plot_scores','plot_distribution','plot_nucleotide_distribution'):
			subprocess.call("read_fastq -i {0} | {1} -T '{0} {1}' -t svg \
			-o {2}/{0}.{1}.svg -x".format(pe,plot,outdir), shell=True)
			...
			subprocess.call("read_fastq -i {0} | {1} -k SEQ_LEN -T '{0} {1}' -t svg \
			-o {2}/{0}.{1}.svg -x".format(pe,plot,outdir), shell=True)
		else:
...
def readStats(infile,outfile):
	"""docstring"""
	print HEADER +'Read library statistics for ' + infile + HEADER
	subprocess.call('prinseq-lite.pl -out_good null -out_bad null -stats_all -fastq {0} > \
	{1}'.format(infile,outfile) , shell=True)
	subprocess.call('prinseq-lite.pl -out_good null -out_bad null -graph_stat qd,ld,gc,ns,pt,ts,de,da,dn,sc \
	-fastq {0} -graph_data {1}.json'.format(infile,outfile) , shell=True)
	subprocess.call('prinseq-graphs-noPCA.pl -html_all -i {0}.json -html_all -o {0}'\
	.format(outfile),shell = True)
	
def assemblyStats(infile,outfile):
	"""docstring"""
	print HEADER +'Assembly statistics for ' + infile + HEADER
	subprocess.call('prinseq-lite.pl -out_good null -out_bad null -stats_all -fasta {0} > \
	{1}'.format(infile,outfile) , shell=True)
	subprocess.call('prinseq-lite.pl -out_good null -out_bad null -graph_stat ld,gc,dn,sc \
	-fasta {0} -graph_data {1}.json'.format(infile,outfile) , shell=True)
	subprocess.call('prinseq-graphs-noPCA.pl -html_all -i {0}.json -html_all -o {0}'\
	.format(outfile),shell = True)
...
def callTrinityAssembler(pe1,pe2,mem,cpu,out):
	...
	subprocess.call('Trinity.pl --full_cleanup --seqType fq --SS_lib_type RF --left {0} \
	--right {1} \
...
def assemblyReduction(id,contig,petrim1,petrim2,cpu,outdir):
	...
	subprocess.call('run_RSEM_align_n_estimate.pl --transcripts {0} --seqType fq \
	--left {1} --right {2} --thread_count {3} --output_dir {4}'.format(contig,petrim1,\
	petrim2,cpu,outdir), shell=True)
	print HEADER + 'Picking isoforms for ' + contig + HEADER
	subprocess.call('pick_isoform_trinity_RSEM.py {0}/RSEM.isoforms.results {1}'\
	.format(outdir,contig), shell=True)
	os.rename('{0}/{1}.trinity.contig.nt.fa.exemplar'.format(outdir,id),\
	'{0}/{1}.trinity.pickH.contig.nt.fa'.format(outdir,id))
	print HEADER + 'CAP3 on ' + '{0}/{1}.trinity.pickH.contig.nt.fa'.format(outdir,id) \
	+ HEADER
	subprocess.call('cap3 {0}/{1}.trinity.pickH.contig.nt.fa -o 200 -p 99'\
	.format(outdir,id), shell=True)
	os.rename('{0}/{1}.trinity.pickH.contig.nt.fa.cap.contigs'.format(outdir,id),\
	'{0}/{1}.trinity.pickH.cap3.contig.nt.fa'.format(outdir,id))
	print HEADER + 'CD-HIT-EST on ' + '{0}/{1}.trinity.pickH.cap3.contig.nt.fa'\
	.format(outdir,id) + HEADER
	subprocess.call('cd-hit-est -i {0}/{1}.trinity.pickH.cap3.contig.nt.fa \
	-o {0}/{1}.trinity.pickH.cap3.cdhit.contig.nt.fa -c 0.99'.format(outdir,id),shell=True)

def chimeraCheck(id,query,output,outdir,db,maxseq,eval,minsize):
	"""docstring"""
	print HEADER +'BLASTX of ' + query + ' vs. ' + db + HEADER
	outformat = "'6 qseqid qlen sseqid slen frames pident nident length mismatch gapopen \
	qstart qend sstart send evalue bitscore'"
	print 'blastx -db {0} -query {1} -evalue {2} -outfmt {3} -out {4} -num_threads={5} \
	-max_target_seqs {6}'.format(db,query,eval,outformat,output,CPU_TOT,maxseq)
	subprocess.call('blastx -db {0} -query {1} -evalue {2} -outfmt {3} -out {4} \
	-num_threads={5} -max_target_seqs {6}'.format(db,query,eval,outformat,output,\
	CPU_TOT,maxseq), shell=True)
	print 120*'#'+'\n'+25*' '+'Detect chimera from ' + output + '\n' + 120*'#'
	subprocess.call('detect_chimera_from_blastx.py ' + outdir, shell=True)
	subprocess.call('cp assemblies/{0}.trinity.pickH.cap3.cdhit.contig.blastx.cut \
	assemblies/{0}.blastx.cut'.format(id),shell=True) #copy/rename cut output for next step
	subprocess.call('cp {0} {0}.before_cut'.format(query),shell=True)
	print 120*'#'+'\n'+25*' '+'Cut chimeras from ' + query  + '\n' + 120*'#'
	subprocess.call('cut_chimera_from_blastx.py {0} {0} {1}'.format(outdir,minsize)\
	,shell=True)
	...
	with cd(TMP_DIR + '/' + M_ID): # project context
		...
		for filename in glob.glob('*.fastq.gz'): # rename fq files and gunzip
			if filename[-10] == '1':
				...
				subprocess.call('chown root:root reads/{}.ncgr.pe1.fq.gz'.format(M_ID), \
				shell=True)
				subprocess.call('gunzip -f reads/' + M_ID + '.ncgr.pe1.fq.gz', shell=True) 
				ncgr_fq1 = M_ID + '.ncgr.pe1.fq' # assign vairable name to ncgr fq file
			elif filename[-10] == '2':
				os.rename(filename,'reads/' + M_ID + '.ncgr.pe2.fq.gz')
				subprocess.call('chown root:root reads/{}.ncgr.pe2.fq.gz'.format(M_ID), \
				shell=True)
		...
		with cd(TMP_DIR + '/' + M_ID + '/reads'): # reads context
			callTrimmomatic(ncgr_fq1,ncgr_fq2,M_ID + '.trim.pe1.fq',M_ID + '.trim.pe2.fq',\
			M_ID + '.trim.se1.fq',M_ID + '.trim.se2.fq', ADAPT, 30, 13)
			
			subprocess.call('interleave-reads.py -o {0}.trim.pe.fq {0}.trim.pe1.fq \
			{0}.trim.pe2.fq'.format(M_ID), shell=True) # Interleave prior to DigitNorm
			
			subprocess.call('interleave-reads.py -o {0}.ncgr.pe.fq {0}.ncgr.pe1.fq \
			{0}.ncgr.pe2.fq'.format(M_ID), shell=True) # Interleave ncgr reads
			
			callDigiNorm(M_ID + '.trim.pe.fq','20','20','4',str(float(MEM_TOT)/4) + 'e9') 
		...
		with cd(TMP_DIR + '/' + M_ID + '/reads'):
			...
			for readtype in readlist:
				...
				readStats('{}.{}.pe.fq'.format(M_ID,readtype),'../stats/' + readtype + '/' + M_ID + '.' + readtype + '.stats')
				#PRODUCE JSON FILE
			...
			os.rename(M_ID + '.trim.diginorm.deconseq.pe.fq.1',M_ID + \
			'.trim.diginorm.deconseq.pe1.fq')
			os.rename(M_ID + '.trim.diginorm.deconseq.pe.fq.2',M_ID + \
			'.trim.diginorm.deconseq.pe2.fq')
			callTrinityAssembler(M_ID + '.trim.diginorm.deconseq.pe1.fq',M_ID + \
			'.trim.diginorm.deconseq.pe2.fq',\
			MEM_TOT + 'G',CPU_TOT,TMP_DIR + '/' + M_ID + '/assemblies/' + M_ID)	
			os.rename('../assemblies/' + M_ID + '.Trinity.fasta','../assemblies/' + M_ID \
			+ '.trinity.contig.nt.fa') 			
		with cd(TMP_DIR + '/' + M_ID):
			
			assemblyReduction(M_ID,'assemblies/' + M_ID + '.trinity.contig.nt.fa','reads/'\
			 + M_ID + '.trim.pe1.fq','reads/' + M_ID + '.trim.pe2.fq',CPU_TOT,'assemblies')
			subprocess.call(\
			'cp assemblies/{0}.trinity.contig.nt.fa assemblies/{0}.trinity.pickH.cap3.cdhit.contig.nt.fa'\
			.format(M_ID), shell=True) #TEST
			chimeraCheck(M_ID,'assemblies/'+ M_ID + '.trinity.pickH.cap3.cdhit.contig.nt.fa'\
			,'assemblies/'+ M_ID + '.trinity.pickH.cap3.cdhit.contig.blastx',\
			'assemblies',BLAST_CHIM_DB,100,0.01,1)
			# rename chimera-checked contig output
			os.rename('assemblies/'+ M_ID + '.cutfa','assemblies/'+ M_ID + \
			'.trinity.pickH.cap3.cdhit.chimera.contig.nt.fa')
			
			# get assembly stats for each step
			assemb_list = ['trinity','trinity.pickH','trinity.pickH.cap3.cdhit',\
			'trinity.pickH.cap3.cdhit.chimera']
			for assemb_type in assemb_list:
				assemblyStats('assemblies/' + M_ID + '.' + assemb_type + '.contig.nt.fa',\
				'stats/' + assemb_type + '/' + M_ID + '.' + assemb_type + '.stats')
			...
			print 'Total pipeline time: ' + str(total_time.seconds) + ' seconds\n'
